...
// Works just commenting because not using in project
...
//public static final int
	// sumPermutationInversionSlowWithCheckForUnSolvability(int[] state) {
	// if (!cached) {
	// System.out.println("Runs once only");
	// int goalStateLength = goalState.length;
	// for (int i = goalStateLength - 1; i >= 0; i--) {
	// if (goalState[i] != 0) {
	// numbersThatFollowANumberInGoalState.put(goalState[i], new
	// ArrayList<Integer>());
	// for (int j = i - 1; j >= 0; j--) {
	// if (goalState[j] != 0) {
	// numbersThatFollowANumberInGoalState.get(goalState[i]).add(goalState[j]);
	...
	// for (int i = 0; i < state.length - 1; i++) {
	// if (state[i] != 0) {
	// ArrayList<Integer> shouldBeOnItsLeft =
	// numbersThatFollowANumberInGoalState.get(state[i]);
	// int count = 0;
	// for (int j = i + 1; j < state.length; j++) {
	// if (shouldBeOnItsLeft.contains(state[j])) {
	// count++;
	...
	// if ((d % 2 != 0 && sumOfPerInv % 2 != 0) || (d % 2 == 0 && sumOfPerInv %
	// 2 == 0)) {
	// // System.out.println("HERE d=" +d + " sumOfPerInv
	// // "+sumOfPerInv);
	// // return 100000;
	// Utility.printMatrix(state);
	// System.out.println("sumOfPerInv " + sumOfPerInv + " row no from bottom "
	// + d);
	...
	// }
	//
	// public static final int getRowCountFromBottom(int
	// indexOfBlankTitleInState, int noOfElementsInState) {
